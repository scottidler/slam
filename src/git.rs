...
pub fn _create_pr(repo: &str, branch: &str, title: &str, body: Option<&str>) -> eyre::Result<()> {
    let mut args = vec![
        "pr", "create",
        "--repo", repo,
        "--head", branch,
    ...
    let output = Command::new("gh")
        .args(&args)
        .output()
        .map_err(|e| eyre::eyre!("Failed to execute gh pr create: {}", e))?;
    if output.status.success() {
        Ok(())
    } else {
        Err(eyre::eyre!(
...
pub fn create_pr(repo_path: &Path, change_id: &str, commit_msg: &str) -> Option<String> {
    let title = format!("SLAM: {}", commit_msg);
    let body = format!(
        r#"{}
...
docs: https://github.com/scottidler/slam/blob/main/README.md"#,
    ...
    match pr_output {
        ...
        Err(err) => {
            warn!("Failed to execute `gh pr create`: {}", err);
            None
        }
    }
}
